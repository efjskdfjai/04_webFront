...
document.querySelector("#btn2").addEventListener("click", () => {
  ...
  // [abcd]
  const regExp1 = /[abcd]/;
  div.innerHTML += "/[abcd]/, apple : " + regExp1.test("apple") + "<hr>";
  div.innerHTML += "/[abcd]/, ssbss : " + regExp1.test("ssbss") + "<hr>";
  div.innerHTML += "/[abcd]/, qwerty : " + regExp1.test("qwerty") + "<hr>";

  // ^ (캐럿)
  const regExp2 = /^group/;
  div.innerHTML += " /^group/, group100 : " + regExp2.test("group100") + "<hr>";
  div.innerHTML += " /^group/, 100group : " + regExp2.test("100group") + "<hr>";

  // $ (달러)

  const regExp3 = /group$/;
  div.innerHTML += " /^group$/, group100 : " + regExp3.test("group100") + "<hr>";
  div.innerHTML += " /^group$/, 100group : " + regExp3.test("100group") + "<hr>";
});
...
// 주민등록번호 유효성 검사하기
document.getElementById("inputPho").addEventListener("keyup", (e) => {
  const span = document.getElementById("imputPhoResult");
