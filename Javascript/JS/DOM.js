...
document.querySelector("#btn1").addEventListener("click", () => {
    ...
    // 3. 첫 번ㅉ내 자식으로 추가(요소.prepend("내용" 또는 노드 또는 요소 n개))
    list[11].prepend("오늘", "점심", "뭐 먹지?");

    // 4. 이전/다음 형제로 추가 (요소.before / 요소.after()) 
    // #test 전 / 후에 내용추가
    test.before("이전입니다" , "ddd");
    test.after("이후입니다");
...
document.querySelector("#btn2").addEventListener("click", () => {
    ...
    console.log(test2.chilren); // HTMLCollection

    // #test2의 첫 번째 자식 요소
    console.log(test2.firstElementchild);

    // #test의 마지막 자식 요소
    console.log(test2.lastElement);
